...
def draw_reward(im, i_epi, rew, rList, n_exploitation, n_exploration):
    ...
    rate_exploitation = n_exploitation / (n_exploitation + n_exploration + 0.00001)
    rate_exploration = 1. - rate_exploitation
    text = 'exploi. rate : ' + '%1.2f' % (rate_exploitation) + '  explor. rate : ' + '%1.2f' % (rate_exploration)
...
rList = []
r_all_pre = 0
n_exploitation, n_exploration = 0, 0
for i in range(num_episodes):
    bunmo = i // exploration_factor
    e = 1. / (bunmo + 1)
    state = env.reset()
    rAll = 0
    done = False
